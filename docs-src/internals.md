...
- **Fork isolation**: by default (`fork_workers: true`), workers stay alive and fork() per test file. Each child gets a copy-on-write clone of the warm interpreter state, runs the test, and exits. This provides both fast startup (the project is loaded once) and full process isolation (no state leakage between files). Set `fork_workers: false` to kill and respawn the worker after every file instead. Fork mode is Linux/macOS only and is automatically disabled on Windows.
